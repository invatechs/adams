...
var handleHook = function(project, notificationSettings, callback) {
  ...
  var branch = !!project.branch ? project.branch : 'master';
  ...
  if(!!project.username && !!project.password) {
    if (isBitBucket) {
      userParams = '--user ' + encodeURIComponent(project.username) + ':' + encodeURIComponent(project.password) + ' ';
    } else if(isGitHub) {
      userParams = '--user "' + encodeURIComponent(project.username) + ':' + encodeURIComponent(project.password) + '" ';
    }
  }
  ...
  if(isBitBucket) {
    ...
    unZip = 'tar -xzf ' + branch + '.tgz -C ' + branch + ' --strip-components=1';
  }

  var cwdAdams = './';
  var cwdTemp = cwdAdams + 'tmp';
  ...
  /*make temp directory if such doesn't exist*/
  child_process.exec('mkdir ' + cwdTemp, {cwd: cwdAdams}, function () {
    ...
    child_process.exec('mkdir ' + gitProjectName, {cwd: cwdTemp}, function () {
      ...
      child_process.exec('rm -rf ' + branch + '; rm -rf ' + branch + '.tgz', {cwd: cwdProject}, function () {
        ...
        child_process.exec('mkdir ' + branch + ';' + downloadArchive, {cwd: cwdProject}, function (errorDownloadArchive) {
          ...
          if (errorDownloadArchive !== null) {
            ...
            console.log('@download git archive success');

            /*unZip project's .tgz archive*/
            child_process.exec(unZip, {cwd: cwdProject}, function (errorUnZip) {
              ...
              if (errorUnZip !== null) {
                ...
                console.log('@unZip git archive success');

                function checkAdamsDefined() {
                  var pckg = require('.' + cwdProjectBranch + '/package.json');
                  if(!!pckg) {
                    if(!!pckg.scripts) {
                      if(!!pckg.scripts.adams) {
                        adamsDefined = true;
                      }
                    }
                  }
                ...
                var adamsDefined = checkAdamsDefined();

                if(adamsDefined) {
                  /*commit 'npm adams' command*/
                  ...
                  child_process.exec('npm install', {cwd: cwdProjectBranch}, function (errorNpmInstall) {
                    ...
                    if (errorNpmInstall !== null) {
                      ...
                      child_process.exec('npm run adams', {cwd: cwdProjectBranch}, function (errorRunAdams) {
